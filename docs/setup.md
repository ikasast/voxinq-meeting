...
| `VOXINQ_VERSION` | Rarely | Pins the image version instead of following `latest`, e.g. `v3.6.0`. Leave it unset to follow the newest stable release. Prereleases never move `latest`, so a beta or rc has to be named here. `v1.5.0` is the last 1.x release — pin it to stay on that line |
