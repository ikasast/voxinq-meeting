...
| `VOXINQ_VERSION` | ほぼ不要 | バージョンを固定したいとき（例 `v3.8.2`）。**未指定なら最新の安定版**（`latest`）を追いかけます。プレリリース版は `latest` に含まれないため、beta や rc を使うにはここで指定します。`v1.5.0` は 1.x 系最後の版で、そこに留まりたい場合に指定します（1.x は NVIDIA GPU が必須です） |
