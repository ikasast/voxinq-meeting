...
議事録の品質はAIモデルの性能に左右されます。より賢いモデルを使うには:

- **LM Studio / vLLM** — ローカルで別のモデルを動かす（APIキー不要）
- **外部GPU（Runpodなど）** — 大きなモデルをレンタルGPUで動かす
- **Anthropic / OpenAI** — クラウドAPIを使う（※この場合、議事録の元データは外部に送信されます）
...
**Q. 複数人で使えますか？**
現状は**個人利用を想定した設計**です（アカウント機能がなく、パスワードは全体で1つです）。

**Q. スマホで録音中に画面を消しても大丈夫ですか？**
画面を点けたままにしてください。端末によっては画面オフでマイクが停止します。
（切断された場合も自動再接続する仕組みは入っています）
