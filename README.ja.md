...
| `VOXINQ_VERSION` | ほぼ不要 | バージョンを固定したいとき（例 `v3.2.0`）。**未指定なら最新の安定版**（`latest`）を追いかけます。プレリリース版は `latest` に含まれないため、beta や rc を使うにはここで指定します。`v1.5.0` は 1.x 系最後の版で、そこに留まりたい場合に指定します（1.x は NVIDIA GPU が必須です） |

> **「安定版」は別に用意されているものではありません。**最新の正式リリースがそのまま安定版です
> — GitHub で *Latest* が付いているもの、`latest` イメージが指すもの、`release` ブランチが
> 指すもの、この3つは同じコミットです（**v3.2.0 時点では v3.2.0**）。`stable` のような別のタグは
