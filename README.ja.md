...
| `VOXINQ_VERSION` | ほぼ不要 | バージョンを固定したいとき（例 `v3.6.0`）。**未指定なら最新の安定版**（`latest`）を追いかけます。プレリリース版は `latest` に含まれないため、beta や rc を使うにはここで指定します。`v1.5.0` は 1.x 系最後の版で、そこに留まりたい場合に指定します（1.x は NVIDIA GPU が必須です） |
...
> ままです。3.x を使うには `VOXINQ_VERSION` で明示してください（例 `v3.6.0`）。
