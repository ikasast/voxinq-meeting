...
| タブ | 主な設定 |
| --- | --- |
| Transcription | Whisperのモデル、認識する言語、専門用語（認識精度が上がります）、マイクモード、日本語訳の有無、**文字起こしを外部エンドポイントで行うかどうか** |
| Speakers | 声紋の登録・削除 |
...
> Settings → Transcription で `/v1/audio/transcriptions` に対応したエンドポイント
> （Groq・OpenAI のほか、**自分の別マシンで動かす whisper サーバー**でも可）を指定すると、
> 同じ1時間が数分で返ってきます。**録音・声紋・話者分離はこのPCに残ります。**
> 対象は会議終了後の一括処理と Re-transcribe だけで、ライブ文字起こしには使えません
> （通信の往復では追いつかないため）。従量課金で、音声1時間あたり $0.25〜0.40 程度です。
