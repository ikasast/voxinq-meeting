...
| 環境 | 文字起こし | リアルタイムに追いつくか | 話者分離 |
| --- | --- | --- | --- |
| **NVIDIA GPU** | faster-whisper（CUDA） | 余裕をもって追いつきます | pyannote（GPU） |
| **Apple Silicon** | whisper.cpp（Metal） | 追いつく見込み（実機未測定） | sherpa-onnx（CPU） |
| **AMD / Intel GPU** | whisper.cpp（**CPU**。GPU は使いません） | 小さいモデルにしないと追いつきません | sherpa-onnx（CPU） |
| **CPU のみ** | whisper.cpp（CPU） | 小さいモデルにしないと追いつきません | sherpa-onnx（CPU） |
...
> **音声の長さの 2.8 倍**かかりました（60 分の会議で約 2.8 時間）。この場合リアルタイム
> 文字起こしは遅れ続け、二度と追いつきません。実時間に間に合うのは `small-q5_1` ですが、
> 精度は目に見えて落ちます。スレッド数を増やしても改善しません（メモリ帯域が上限のため）。
>
