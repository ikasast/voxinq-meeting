...
describe("the recording control", () => {
  ...
  it("keeps the guards that came with it", () => {
    const bar = src.slice(bottomBar);
    // Starting is refused while the meeting has ended, while another GPU job runs, and from
    // outside the tailnet where the STT service cannot be reached at all. Stopping is always
    // allowed — that is what `&& !active` in startBlocked is for.
    expect(bar).toContain("disabled={(external && !active) || startBlocked}");
    expect(src).toContain("const startBlocked = ended || (gpu.busy && !active)");
  });
