...
export default async function PrintMinutes({ params }: { params: Promise<{ id: string }> }) {
  ...
  const duration = meeting.endedAt
    ? formatDurationIn(locale, meeting.endedAt.getTime() - meeting.startedAt.getTime())
    : null;
    ...
    <main className="print-sheet mx-auto max-w-3xl px-6 py-8">
      ...
      <div className="no-print mb-6 flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-[var(--text-muted)]">
          {t("Choose “Save as PDF” as the destination to keep a copy.")}
        </span>
      </div>
      ...
      {minutes ? (
        ...
        <p className="text-sm text-[var(--text-muted)]">
          {t("No minutes have been generated for this meeting yet.")}
        </p>
      )}

      <footer className="mt-10 text-right text-[10px] text-[var(--text-muted)]">
        {t("Exported from Voxinq Meeting on {when}", {
          when: formatDateTimeIn(locale, new Date()),
        })}
      </footer>
    </main>
  );
